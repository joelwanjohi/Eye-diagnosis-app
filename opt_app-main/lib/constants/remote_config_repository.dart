...
class RemoteConfigRepository {
  ...
  static Future _updateConfigs(RemoteConfigUpdate remoteConfigUpdate) async {
    ...
    medicalHealthList =
        Set.from(jsonDecode(_remoteConfig.getString(kmedicalHealth)))
            .map((e) => Complaint.fromJson(e))
            .toList();
    ocularHealthList =
        Set.from(jsonDecode(_remoteConfig.getString(kocularHealth)))
            .map((e) => Complaint.fromJson(e))
            .toList();
  }
  ...
  static Future<dynamic> _getDefaultData() async {
    final String response =
        await rootBundle.loadString('assets/remote_config_defaults.json');
